...
## 2026-08-10 — Relationship-aware AI planning

- Audited existing Goal/Roadmap/Stage/Task/Checklist foreign keys and reused them without a migration.
- Added bounded owner-scoped entity resolution, normalized duplicate ranking, relationship confidence, source provenance and hierarchy validation.
- Added Goal reuse, Roadmap-to-Goal linking, transactional Roadmap extension, Task hierarchy linking and Task-to-Checklist linking.
- Added responsive relationship preview/selectors and entity-page Ask AI quick actions.
- Added existing Task IDs/parents to Daily/Weekly planning context and regression coverage for duplicate, ownership, hierarchy and Create New override behavior.


## 2026-08-09 — Real-provider AI, mobile AI and Note delete (external verification blocked)

- Added pinned OpenAI/Groq/Gemini AI SDK providers, centralized priority/fallback, timeout, normalized errors, usage/attempt metadata and a bounded Server Action rate limit.
- Routed chat and structured proposals through the orchestrator while preserving runtime validation and explicit confirmation before Feature Service writes.
- Added AI to mobile navigation, responsive sticky chat input and provider/fallback indicators.
- Added owner-scoped Note soft-delete confirmation to list/detail without a migration.

## 2026-08-09 — QA release hardening and deployment (In Progress)

- Upgraded Next.js to 16.3.0, resolved production dependency advisories, added global security headers and improved keyboard/contrast accessibility.
- Reconciled 21/21 migration records; verified forced RLS on 28/28 public tables, role behavior, private Storage policies and separate database/object export rehearsal.
- Deployed and verified `https://coffeehub.id.vn`, including HTTPS, protected redirects, PWA/offline behavior and desktop/mobile accessibility.
- Release remains blocked because Supabase public signup is enabled, leaked-password protection is disabled and successful owner/admin Auth E2E cannot be completed with the rejected credential.

## 2026-08-09 — AI Personal Assistant completed

- Added owner-scoped multi-turn conversations/messages and AI settings with optional history, retention, forced RLS and separate confirmed history deletion.
- Added transparent bounded context, daily plan, weekly review and note summary modes with pending/streaming/error/retry/cancel UI states.
- Added Event and Note create/update proposals to the existing editable, explicit-confirmation, idempotent action pipeline.
- Added AI settings at `/app/settings/ai`, including AI disable and context minimization controls.
- Applied and verified migration `20260809135819_ai_personal_assistant`; Prettier, Prisma, lint, typecheck, 80 tests and production build pass.

## 2026-08-09 — AI Goal Assistant completed

- Completed Checklist proposal plus editable draft preview, discard and single-use owner confirmation at `/app/ai`.
- Added server re-validation, version/hash binding, atomic idempotency, full audit and real result links; providers remain database-free.
- Added and applied owner-scoped FORCE RLS persistence for AI drafts/audit. Verified 77/77 tests, lint, typecheck, production build and database rollback for the Goal → Roadmap → Stage batch.

## 2026-08-09 — AI Task proposal completed

- Added bounded Task structured validation and re-validation through the existing Task form schema.
- Kept relation IDs server-controlled and empty at proposal time; stage reference is display-only and save remains disabled.
- Passed Prettier, lint, typecheck, 74/74 tests, production build and authenticated desktop/mobile browser verification.

## 2026-08-09 — AI Roadmap proposal completed

- Added bounded Roadmap/Stage structured validation and re-validation through existing Roadmap/Milestone form schemas.
- Expanded `/app/ai` preview with estimated duration, stage descriptions/durations and proposed task counts while preserving the disabled save boundary.
- Passed Prettier, lint, typecheck, 72/72 tests, production build and authenticated desktop/mobile browser verification.

## 2026-08-09 — AI Goal proposal completed

- Completed owner-only Goal proposal generation at `/app/ai` with AI runtime validation, mapping and re-validation through the existing Goal form schema.
- Confirmed the proposal remains unsaved and the database save control is disabled, preserving D-013's confirmation boundary.
- Passed Prettier, lint, typecheck, 70/70 tests, production build and authenticated desktop/mobile browser verification with zero overflow or browser errors.

## 2026-08-08 — Notifications delivery completed

- Added owner reminder/preferences management with recurrence, entity deep links, cancellation and explicit browser/iPhone PWA limitations.
- Applied and verified additive Supabase migration, forced RLS, owner/admin policies and domain constraints.
- Passed Prettier, Prisma, lint, typecheck, 65/65 tests, production build and authenticated desktop/mobile E2E; test reminders were removed with zero residue.

## 2026-08-08 — Checklists delivery completed

- Added protected Checklist list/create/detail flows with search, edit/archive, item add/delete, optimistic toggle rollback and progress derived only from active items.
- Established D-014: Checklist is independent from Task/Subtask, may reference at most one owner-scoped Goal/Roadmap/Task and does not affect Goal or Dashboard progress.
- Applied and verified three additive Supabase development migrations for Checklist/ChecklistItem, RLS optimization and database domain constraints; new tables have forced RLS and owner/admin policies.
- Passed Prettier check, Prisma generate/validate, lint, typecheck, 62/62 tests, production build and authenticated desktop/mobile E2E; the exact E2E Checklists were archived with zero active residue.
