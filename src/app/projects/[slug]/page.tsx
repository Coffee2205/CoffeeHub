...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    ...
    return project
      ? { title: `${project.title} | CoffeeHub`, description: project.summary }
      : { title: "Dự án | CoffeeHub" };
  } catch {
    return { title: "Dự án | CoffeeHub" };
  }
}
    ...
    return (
      <PublicShell>
        <div className="mx-auto max-w-4xl px-5 py-28 sm:px-8">
          <h1 className="text-4xl font-semibold">Chưa thể tải dự án</h1>
  ...
  return (
    <PublicShell>
      <article className="mx-auto max-w-4xl px-5 py-20 sm:px-8 sm:py-28">
        <Link href="/projects" className="text-sm text-cyan-300">
          ← Tất cả dự án
        </Link>
        <p className="mt-12 text-xs uppercase tracking-[.22em] text-blue-300">
          {project.role ?? "Project case study"}
        ...
        <div className="mt-10 flex flex-wrap gap-2">
          {project.techStack.map((tech) => (
            <span
              key={tech}
              className="rounded-full border border-blue-300/20 px-4 py-2 text-sm text-blue-100"
            >
              {tech}
            </span>
          ))}
        </div>
        <div className="mt-16 whitespace-pre-line border-t border-white/10 pt-12 text-base leading-8 text-slate-300">
          {project.description}
        </div>
        <div className="mt-12 flex flex-wrap gap-3">
          {project.liveUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full bg-blue-500 px-6 py-3 font-medium"
            >
              Mở sản phẩm ↗
            </a>
          ) : null}
          {project.githubUrl ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="rounded-full border border-white/15 px-6 py-3 font-medium"
            >
              Xem mã nguồn ↗
            </a>
          ) : null}
