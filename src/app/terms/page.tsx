...
export const metadata: Metadata = {
  title: "Terms | CoffeeHub",
  description: "Điều khoản sử dụng website CV.",
};
export default async function TermsPage() {
  let data: Awaited<ReturnType<typeof getPublicLegalPage>> | null = null;
  try {
    data = await getPublicLegalPage("terms");
  } catch (error) {
    console.error("Unable to load terms content", error);
  }
  return (
    <PublicShell>
      <PageIntro
        eyebrow="Legal"
        title="Terms"
        description="Điều khoản áp dụng khi truy cập website CV và portfolio."
      />
      <div className="mx-auto max-w-4xl space-y-8 px-5 py-20 sm:px-8">
        {!data ? (
          ...
          <PublicNotice
            title="Terms đang được cập nhật"
            body="Owner chưa publish nội dung terms trong CMS."
          />
        )}
      </div>
    </PublicShell>
