...
export default async function GoalRoadmapPage({
  params,
  searchParams,
}: {
  ...
  const goal = await getGoalRoadmaps(user.id, goalId);
  if (!goal) notFound();
  return (
    <div className="space-y-6">
      ...
      {query.saved ? (
        ...
        >
          Thay đổi roadmap đã được lưu.
        </p>
      ) : null}
      {goal.roadmaps.length ? (
        <div className="grid gap-6">
          {goal.roadmaps.map((roadmap) => (
            <RoadmapBoard key={roadmap.id} goalId={goal.id} roadmap={roadmap} />
          ))}
        </div>
      ) : (
        <EmptyState
          title="Goal chưa có Roadmap"
          description="Tạo roadmap để chia Goal thành các milestone có thể thực hiện."
        />
      )}
      <form
        action={createRoadmapAction.bind(null, goal.id)}
        className="grid gap-4 rounded-lg border border-border bg-surface p-5 sm:p-6"
      >
        <h2 className="text-lg font-semibold">Tạo Roadmap</h2>
        <label className="grid gap-2">
          <span className="text-sm font-medium">Tên roadmap</span>
          <Input
            name="title"
            required
            maxLength={180}
