  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <Badge variant={goal.status === "ACTIVE" ? "success" : "neutral"}>
            {goal.status}
          </Badge>
        ...
        <div className="grid content-start gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Tính từ Task chưa xóa và không bị hủy gắn với Goal.
              </CardDescription>
            </CardHeader>
            <div className="flex items-end justify-between">
              <strong className="font-mono text-4xl">{progress}%</strong>
              <span className="text-sm text-muted">
                {completed}/{goal.tasks.length} Task
              </span>
            </div>
            <div
              className="mt-4 h-3 overflow-hidden rounded-full bg-background-tertiary"
              role="progressbar"
            ...
            <p className="mt-4 text-sm text-muted">
              {goal.roadmaps.length} roadmap liên quan
            </p>
            <Link
              href={`/app/goals/${goal.id}/roadmap`}
              className="mt-4 inline-flex min-h-11 items-center rounded-sm border border-border-strong bg-surface-strong px-4 text-sm font-semibold hover:border-primary-hover"
            >
              Mở Roadmap →
            </Link>
          </Card>
          <Card>
            <CardHeader>
              <CardTitle>Tiêu chí thành công</CardTitle>
            </CardHeader>
            {criteria.length ? (
              ...
              <p className="text-sm text-muted">
                Chưa có tiêu chí. Thêm tiêu chí trong form để Goal có định nghĩa
                hoàn thành rõ ràng.
              </p>
            )}
          </Card>
