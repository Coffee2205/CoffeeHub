...
const valid = (kind: string): kind is ResumeKind =>
  RESUME_KINDS.some((item) => item === kind);
export default async function EditResumePage({
  ...
  const item = await getResumeItem(kind, itemId);
  if (!item) notFound();
  return (
    <main className="mx-auto max-w-4xl px-[var(--page-gutter)] py-10">
      <h1 className="text-3xl font-semibold">Sửa nội dung</h1>
      <ResumeForm
        kind={kind}
        item={item}
        action={updateResumeAction.bind(null, kind, itemId)}
        error={query.error}
      />
    </main>
  );
