...
} from "@/features/profile/profile-avatar.actions";
  ...
  return (
    <main className="mx-auto max-w-4xl px-[var(--page-gutter)] py-10">
      ...
      <p className="mt-2 text-foreground-secondary">
        Nội dung hồ sơ công khai; draft và hidden không xuất hiện qua Data API.
      </p>

      <form
        action={saveProfileAction}
        className="mt-6 grid gap-5 rounded-lg border border-border bg-surface p-5"
      >
        {query.error ? (
      ...
      <section className="mt-6 rounded-lg border border-border bg-surface p-5">
        ...
        <p className="mt-1 text-sm text-foreground-secondary">
          JPEG, PNG hoặc WebP; tối đa 5 MB. Avatar chỉ xuất hiện công khai khi
          Profile ở trạng thái Published.
        </p>
        {query.avatarError ? (
        ...
        <div className="mt-5 grid gap-6 md:grid-cols-[12rem_1fr]">
          <div className="relative aspect-square overflow-hidden rounded-2xl border border-border bg-background-secondary">
            {avatarUrl ? (
              <Image
                src={avatarUrl}
                alt={profile?.avatarAlt ?? "Avatar Profile"}
                fill
                sizes="192px"
                className="object-cover"
              />
            ) : (
              <div className="grid h-full place-items-center px-4 text-center text-sm text-foreground-secondary">
                Chưa có avatar
              </div>
            )}
          </div>
          <div>
            <form action={uploadProfileAvatarAction} className="grid gap-4">
              ...
              <label className="grid gap-2 text-sm">
                ...
                <Input
                  ...
                  defaultValue={
                    profile?.avatarAlt ??
                    (profile?.displayName
                      ? `Ảnh chân dung ${profile.displayName}`
                      : "")
                  }
                />
              </label>
              <div>
                <Button type="submit">
                  {profile?.avatarPath ? "Thay avatar" : "Tải avatar"}
                </Button>
              </div>
            </form>
            {profile?.avatarPath ? (
              <div className="mt-4">
                <DeleteAvatarButton action={deleteProfileAvatarAction} />
              </div>
            ) : null}
          </div>
        </div>
