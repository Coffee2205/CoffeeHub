...
export default async function EditSiteContentPage({ params, searchParams }: { params: Promise<{ kind: string; itemId: string }>; searchParams: Promise<{ error?: string }> }) { const [{ kind, itemId }, query] = await Promise.all([params, searchParams]); if (!valid(kind)) notFound(); const item = await getSiteContentItem(kind, itemId); if (!item) notFound(); return <main className="mx-auto max-w-4xl px-[var(--page-gutter)] py-10"><h1 className="text-3xl font-semibold">Sửa nội dung website</h1><SiteContentForm kind={kind} item={item} action={updateSiteContentAction.bind(null, kind, itemId)} error={query.error} /></main>; }
