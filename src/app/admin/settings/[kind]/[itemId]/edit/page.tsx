...
import {
  MANAGED_KINDS,
  type ManagedKind,
} from "@/features/site-settings/site-settings.schema";
...
export default async function EditManagedPage({
  ...
  const item = await getManagedItem(kind, itemId);
  if (!item) notFound();
  return (
    <main className="mx-auto max-w-4xl px-[var(--page-gutter)] py-10">
      <h1 className="text-3xl font-semibold">
        Sửa {kind === "link" ? "liên kết" : "FAQ"}
      </h1>
      <ManagedForm
        kind={kind}
        item={item}
        action={updateManagedAction.bind(null, kind, itemId)}
        error={query.error}
      />
    </main>
  );
