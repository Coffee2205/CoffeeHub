  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-border bg-background-secondary/95 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
      <Link href="/app/dashboard" className="flex min-h-11 items-center gap-3 rounded-sm px-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-hover">
        ...
        <span>
          <span className="block font-semibold tracking-[-0.02em]">CoffeeHub</span>
      ...
      <nav className="mt-7 flex-1 space-y-6 overflow-y-auto" aria-label="Điều hướng workspace">
        ...
        <div key={group.label}>
          <p className="mb-2 px-3 text-[0.6875rem] font-semibold uppercase tracking-[0.16em] text-muted">{group.label}</p>
      ...
      <div className="mt-4 flex min-h-12 w-full items-center gap-3 rounded-sm border border-border bg-surface-subtle px-3 text-left">
        <span className="grid size-8 place-items-center rounded-full bg-background-tertiary text-xs font-semibold text-primary-hover">{user.displayName?.slice(0, 1).toUpperCase() ?? "C"}</span>
        <span className="min-w-0">
          <span className="block truncate text-sm font-medium">{user.displayName ?? "Thành viên CoffeeHub"}</span>
          <span className="block text-xs text-muted">{user.isAdmin ? "Quản trị viên" : "Thành viên"}</span>
        </span>
      </div>
    </aside>
  );
