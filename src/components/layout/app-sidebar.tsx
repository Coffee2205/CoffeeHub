...
type UserSummary = { email: string | null; isAdmin: boolean };
...
export function AppSidebar({ user }: { user: UserSummary }) {
  return (
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-border bg-background-secondary/95 px-4 py-5 backdrop-blur-xl lg:flex lg:flex-col">
      ...
      <div className="mt-4 flex min-h-12 w-full items-center gap-3 rounded-sm border border-border bg-surface-subtle px-3 text-left"><span className="grid size-8 place-items-center rounded-full bg-background-tertiary text-xs font-semibold text-primary-hover">{user.email?.slice(0, 1).toUpperCase() ?? "C"}</span><span className="min-w-0"><span className="block truncate text-sm font-medium">{user.email ?? "Tài khoản CoffeeHub"}</span><span className="block text-xs text-muted">{user.isAdmin ? "Quản trị viên" : "Thành viên"}</span></span></div>
    </aside>
  );
}
