...
export function ResumeForm({
  ...
  const date = (input?: Date | null) => input?.toISOString().slice(0, 10) ?? "";
  return (
    <form
      action={action}
    ...
    >
        ...
        <>
          ...
          <Field label="Ngày bắt đầu">
            <Input
              ...
              type="date"
              required
              defaultValue={date(item?.startedAt)}
            />
          </Field>
          <Field label="Ngày kết thúc">
            <Input
              name="endedAt"
              type="date"
              defaultValue={date(item?.endedAt)}
            />
          </Field>
        </>
      ) : null}
      ...
      <div className="md:col-span-2">
        <Button type="submit">
          {item ? "Lưu thay đổi" : `Tạo ${labels[kind]}`}
        </Button>
      </div>
    </form>
  );
