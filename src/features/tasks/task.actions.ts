...
import { parseTaskForm } from "./task.schema";
...
export async function createTaskAction(form: FormData) {
  ...
  if (!task)
    return invalid(
      "/app/tasks/new",
      "Quan hệ Goal, Roadmap hoặc Stage không hợp lệ.",
    );
  revalidatePath("/app/tasks");
  redirect(`/app/tasks/${task.id}?saved=created`);
}
export async function updateTaskAction(id: string, form: FormData) {
  ...
  if (!task)
    return invalid(
      `/app/tasks/${id}`,
      "Task hoặc quan hệ đã chọn không hợp lệ.",
    );
  revalidatePath("/app/tasks");
  redirect(`/app/tasks/${id}?saved=updated`);
}
export async function archiveTaskAction(id: string) {
  const user = await requireUser();
  const result = await archiveTask(user.id, id);
  if (!result.count) invalid(`/app/tasks/${id}`, "Task không tồn tại.");
  revalidatePath("/app/tasks");
  redirect("/app/tasks?archived=1");
}
export async function setTaskCompletedAction(id: string, completed: boolean) {
  const user = await requireUser();
  const result = await setTaskCompleted(user.id, id, completed);
  if (!result.count) return { ok: false, error: "Không thể cập nhật Task." };
