  ...
  const [
    ...
    prisma.task.findMany({
      where: {
        ...
        dueAt: { gte: ranges.todayStart, lt: ranges.tomorrowStart },
      },
    ...
    prisma.goal.findMany({
      ...
      select: {
        ...
        deadline: true,
        tasks: {
          where: { deletedAt: null, status: { not: TaskStatus.CANCELLED } },
          select: { status: true },
        },
      },
    }),
    prisma.event.findMany({
      where: {
        userId,
        deletedAt: null,
        startsAt: { gte: ranges.now, lt: ranges.upcomingEnd },
      },
    ...
    prisma.task.findMany({
      where: {
        ...
        dueAt: { gte: ranges.weekStart, lt: ranges.nextWeekStart },
      },
      select: { status: true },
    }),
    prisma.task.count({
