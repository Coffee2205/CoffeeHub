...
export async function getDashboardData(
  ...
  return {
    ...
    todayTasks: snapshot.todayTasks.filter(
      (task): task is typeof task & { dueAt: Date } => task.dueAt !== null,
    ),
    overdueTasks: snapshot.overdueTasks.filter(
      (task): task is typeof task & { dueAt: Date } => task.dueAt !== null,
    ),
    activeGoals: snapshot.activeGoals.map((goal) => ({
      ...
      completedTasks: goal.tasks.filter(
    ...
    weeklyProgress: calculateProgress(
      snapshot.weeklyTasks.map((task) => task.status),
    ),
  };
}
