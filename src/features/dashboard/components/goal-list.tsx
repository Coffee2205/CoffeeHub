...
export function GoalList({ goals }: { goals: DashboardGoal[] }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Goal đang hoạt động</CardTitle>
        <CardDescription>
          Tiến độ dựa trên Task chưa xóa và không bị hủy của từng Goal.
        </CardDescription>
      </CardHeader>
      {goals.length === 0 ? (
        <EmptyState
          className="min-h-44"
          title="Bạn chưa có Goal đang hoạt động"
          description="Goal ở trạng thái ACTIVE sẽ xuất hiện tại đây."
        />
      ) : (
            ...
            return (
              <li key={goal.id}>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted">
                      {goal.deadline
                        ? `Hạn ${formatUtcDate(goal.deadline)} UTC`
                        : "Chưa có deadline"}
                    </p>
                  </div>
                ...
                <p className="mt-2 text-xs text-muted">
                  {goal.completedTasks}/{goal.totalTasks} Task hoàn thành
                </p>
              </li>
            );
          })}
