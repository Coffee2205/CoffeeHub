...
export function parseGoalForm(form: FormData): {
  data?: GoalInput;
  errors: string[];
} {
  ...
  const priority = GOAL_PRIORITIES.find(
  ...
  if (!title || title.length > 180)
    errors.push("Tiêu đề phải có từ 1 đến 180 ký tự.");
  ...
  if (
    ...
    errors.push("Tối đa 20 tiêu chí, mỗi tiêu chí không quá 240 ký tự.");
