...
export function ManagedForm({
  ...
  return (
    <form
      action={action}
      className="mt-8 grid gap-5 rounded-lg border border-border bg-surface p-6"
    >
      {error ? (
        <p
          role="alert"
      ...
      {kind === "link" ? (
        <>
          ...
          <Field label="Vị trí">
            <select
              name="kind"
              defaultValue={link?.kind ?? "NAVIGATION"}
              className="input"
            >
          ...
          <label className="flex items-center gap-3 text-sm">
            <input
              type="checkbox"
              name="openNewTab"
              defaultChecked={link?.openNewTab}
            />{" "}
            Mở trong tab mới
          </label>
        </>
      ...
      <div className="flex gap-3">
        <button className="rounded-sm bg-primary-control px-5 py-3 font-semibold text-white">
          Lưu
        </button>
        <Link
          href="/admin/settings"
          className="rounded-sm border border-border-strong px-5 py-3 font-semibold"
        >
          Hủy
        </Link>
      </div>
