...
const integer = (value: string | undefined, fallback: number) => { const parsed = Number(value); return Number.isInteger(parsed) && parsed > 0 ? parsed : fallback; };
const provider = (value: string | undefined, fallback: AIProviderName): AIProviderName => providerNames.includes(value as AIProviderName) ? value as AIProviderName : fallback;

export function getAIConfig() {
  const order = (process.env.AI_PROVIDER_FALLBACK_ORDER ?? "openai,groq,gemini,mock").split(",").map((item) => provider(item.trim(), "mock"));
  return {
    ...
    models: { openai: process.env.OPENAI_DEFAULT_MODEL || "not-configured", groq: process.env.GROQ_DEFAULT_MODEL || "not-configured", gemini: process.env.GEMINI_DEFAULT_MODEL || "not-configured", mock: "mock-planner-v1" },
