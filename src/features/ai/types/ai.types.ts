...
export type RoadmapStageProposal = {
  title: string;
  description?: string;
  order: number;
...
export type DailyPlanProposal = {
  ...
  timeBlocks: TimeBlockProposal[];
  assumptions: string[];
  warnings: string[];
};
export type WeeklyPlanProposal = DailyPlanProposal;
export type GoalReviewProposal = {
  summary: string;
  progress: number;
  wins: string[];
  risks: string[];
...
export type PlanningPreferencesContext = {
  preferredDays?: string[];
  maximumTasksPerDay?: number;
};
...
export type AIUsageLog = AIUsage & {
  proposalId?: string;
  metadata: AIProviderMetadata;
};
