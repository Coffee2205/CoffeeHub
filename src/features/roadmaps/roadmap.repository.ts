...
export async function createStage(
  ...
  return prisma.$transaction(async (tx) => {
    const roadmap = await tx.roadmap.findFirst({
      where: { id: roadmapId, goalId, userId, deletedAt: null },
      select: { id: true },
    });
    if (!roadmap) return null;
    const last = await tx.roadmapStage.findFirst({
...
export async function moveStage(
  ...
  const prisma = getPrisma();
  return prisma.$transaction(async (tx) => {
    const stages = await tx.roadmapStage.findMany({
      where: {
        roadmapId,
        userId,
        deletedAt: null,
    ...
    const highest = await tx.roadmapStage.findFirst({
      where: { roadmapId, userId },
      orderBy: { position: "desc" },
      select: { position: true },
    });
    ...
    await tx.roadmapStage.update({
      where: { id: target.id },
      data: { position: current.position, version: { increment: 1 } },
    });
    await tx.roadmapStage.update({
      where: { id: current.id },
      data: { position: target.position, version: { increment: 1 } },
    });
    return true;
  });
}
