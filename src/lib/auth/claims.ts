...
export function toCurrentUser(claims: unknown): CurrentUser | null {
  ...
  const appMetadata = "app_metadata" in claims && typeof claims.app_metadata === "object" && claims.app_metadata !== null
    ? claims.app_metadata
    : null;
  const isAdmin = appMetadata !== null && "role" in appMetadata && appMetadata.role === "admin";
