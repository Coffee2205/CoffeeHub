...
The provider registry owns server-only OpenAI/Groq/Gemini AI SDK adapters and the explicit Mock Provider, while a single config boundary reads environment values. `/app/ai` uses authenticated Server Actions for runtime-validated proposals, editable relationship previews and single-use confirmation before persistence.
...
## 20. AI planning graph

`planning-entity-resolver.ts` queries a bounded owner-scoped candidate graph from existing Goal, Roadmap, RoadmapStage and Task tables. `planning-relationship.ts` owns deterministic normalization, duplicate ranking and hierarchy validation. The proposal JSONB envelope stores generated `data`, server-derived `relationship`, and `sourceContext`; no schema migration is required.

Confirmation semantics:

- `CREATE_NEW`: create a standalone/new graph only after explicit owner choice;
- `LINK_EXISTING`: create the proposed entity with validated existing parents, or reuse a duplicate Goal;
- `EXTEND_EXISTING`: append proposed Roadmap stages/tasks transactionally;
- `UPDATE_EXISTING`: reserved until an explicit diff/update service is delivered.

Checklist continues to enforce D-014: at most one direct Goal, Roadmap or Task parent. Task may carry the complete Goal/Roadmap/Stage chain when hierarchy matches.
