...
Planning proposals carry server-derived relationship intent: `CREATE_NEW`, `LINK_EXISTING`, `EXTEND_EXISTING` or reserved `UPDATE_EXISTING`, plus confidence and source provenance. Goal duplicate matches default to using the existing Goal; Roadmap can link an existing Goal or extend an existing Roadmap; Task can link Goal/Roadmap/Stage; Checklist can link one existing Task (or another single D-014 context). The owner may change these selections before saving. `RELATIONSHIP_CONFLICT` blocks missing ownership, stale entities and cross-hierarchy combinations.
