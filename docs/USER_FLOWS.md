...
## AI planning with existing entities

```text
Owner request or entity-page quick action
→ server loads bounded owner candidates
→ duplicate/relationship resolution
→ AI drafts content without UUIDs
→ preview shows Goal/Roadmap/Stage/Task relationship
→ owner changes parent or chooses Create New Anyway
→ save refreshes confirmation
→ server revalidates ownership + hierarchy
→ confirm links, extends or creates through the existing domain tables
```

Ambiguous matches require an explicit selection. Extending a Roadmap shows the proposed Stage/Task additions and never silently creates another Goal/Roadmap. Daily/Weekly chat plans reference existing Task IDs and do not create duplicate Task records.
